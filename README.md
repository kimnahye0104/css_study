...
* `img태그`가 들어가는 부모요소에 크기 입력 시, 꼭 필요한 경우가 아니면 width, height 둘다 입력하지 않고 width 값만 입력하여 height 값은 비율에 맞게 자동으로 설정되게 처리한다. (img태그는 inline태그여도 원본 이미지 px 고유값을 가지고 있기 때문에 부모의 크기가 변경되도 상대값으로 따라가지 않는다. => 따라서 강제로 따라가게끔 width: 100%; 입력을 한다.)
### box-shadow
...
* **프린트** : `@media print {body,html{background:white}}`
* **스크린 크기에 따른 디자인(css) 변경** : `@media screen and (max-width:100px) {선택자{속성:값;}}`
## 반응형 제작 준비
### 1-각 디바이스 해상도 이해하기
...
* 보통은 반응형으로 만든다.
## CSS 상태 선택자 (동적기능 속성)
* `선택자:hover { 속성 : 값 ; }` : 마우스 올렸을 때 - 상태 변화
* `선택자:checked { 속성 : 값 ; }` : 클릭했을 때 - 상태 변화
## color 바꾸는 속성
1) `background-color` : 배경 색상
2) `color` : 글자 색상
3) `filter` : img의 고유 색상(원본 색상)에 색반전, 투명도, 채도, 세피아를 주어 색상을 변경한다.
## Grid 란?
### flex와 grid 비교
* flex : 1차원 / grid : 2차원
### grid의 부모 자식 관계 (적용 방식)
* flex와 마찬가지로, 부모(container)-자식(item)